...
Usage:
    ...
    python scripts/run_baseline.py (runs baseline on real lunar observation pair)
"""
...
def run_pair(pair_row: dict, config: dict, output_dir: Path) -> dict:
    pair_id = pair_row.get("pair_id", "DEV-REAL")
    ...
    # 5. Estimate Geometry
    reproj_thresh = config.get("geometry", {}).get("reprojection_threshold_px", 5.0)
    ...
    result = {
        "pair_id": pair_id,
        "success": bool(inliers_count >= 15 and rmse <= 4.0 and spatial_coverage >= 0.25),
...
def main():
    parser = argparse.ArgumentParser(description="Run classical registration baseline")
    ...
    # Check if user specified manifest mode
    if manifest_path.exists() and (args.pair or args.all):
        ...
        # Default mode: execute on real Chandrayaan-2 lunar imagery
